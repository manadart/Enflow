...
namespace Enflow.Base
{
    ...
    {
        IWorkflow<T> Get<T>(string name) where T : IEnflowModel<T>;
    }
    ...
    public abstract class WorkflowFactory : IWorkflowFactory
    {
        ...
        protected void Register<T>(string name, Func<IWorkflow<T>> resolver) where T : IEnflowModel<T>
        {
            _registrations.Add(name, resolver);
        }

        public IWorkflow<T> Get<T>(string name) where T : IEnflowModel<T>
        {
